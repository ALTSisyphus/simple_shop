...
- владелец товара назначается автоматически при создании;
- редактирование и удаление ограничены владельцем или модератором;
- для товара поддерживается статус публикации и снятие с публикации по праву;
...
просмотров и Bootstrap-оформление формы, включая чекбокс публикации. Изменять
раздел блога могут только пользователи с соответствующими правами
контент-менеджера.


Команда `setup_groups` идемпотентно создаёт группы «Модератор продуктов» и
«Контент-менеджер» и назначает им права домашнего задания. Модератор продуктов
получает `can_unpublish_product`, `change_product` и `delete_product`; право
`change_product` необходимо, чтобы модератор мог редактировать товары согласно
критериям оценки.