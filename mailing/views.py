...
def dashboard(request):
    key = f"mailing_dashboard_{request.user.pk}"
    ...
    if result is None:
        ...
        result = {
            ...
            "successful": sum(
                m.attempts.filter(status="Успешно").count()
                for m in mailings
            ),
            "failed": sum(
                m.attempts.filter(status="Не успешно").count()
                for m in mailings
            ),
        }
        cache.set(key, result, 900)

    return render(request, "mailing/dashboard.html", result)
